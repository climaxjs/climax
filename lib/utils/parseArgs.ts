...
export default function parseArgs(commands: string[], args: string[]): ParsedArgs {
  const parsedArgs = yargs.parse(args)

  const options = { ...R.omit(['_', '$0'], parsedArgs) } as BNSObject

  // If there is at least one value (that could be a command) and the first value matches the first
  // argument, it may be a command
  if (parsedArgs._.length > 0 && parsedArgs._[0] === args[0] && commands.includes(args[0])) {
