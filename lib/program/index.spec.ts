...
describe(`Program`, () => {
  ...
  describe(`#version`, () => {
    ...
    it(`should fail with a valid version starting with a "v"`, () =>
      expect(() => program.version('v1.2.3')).toThrow(errors.dictionary.ERR_PROGRAM_VERSION_VALIDATION_V))
    it(`should fail with a valid version starting with a "V"`, () =>
      expect(() => program.version('V1.2.3')).toThrow(errors.dictionary.ERR_PROGRAM_VERSION_VALIDATION_V))
    it(`should fail with a some invalid versions`, () => {
      expect(() => program.version('0')).toThrow(errors.dictionary.ERR_PROGRAM_VERSION_VALIDATION_SEMVER)
      expect(() => program.version('1')).toThrow(errors.dictionary.ERR_PROGRAM_VERSION_VALIDATION_SEMVER)
      expect(() => program.version('0.0')).toThrow(errors.dictionary.ERR_PROGRAM_VERSION_VALIDATION_SEMVER)
      expect(() => program.version('1.2')).toThrow(errors.dictionary.ERR_PROGRAM_VERSION_VALIDATION_SEMVER)
    })
